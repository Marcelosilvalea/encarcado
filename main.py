...
# Configure a API do Google Gemini apenas se houver chave em env
GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY", "AIzaSyDFLyFuZma4hJCVmBuJsljHU03BtDJvZ50")  # não deixar valor padrão hardcoded
...
# Banco de perguntas fallback por nível (caso a IA falhe)
QUESTIONS_DB = {
    1: [  # Fácil - 15 perguntas
...
def generate_ai_question(level: int, question_num: int, answered_questions: list) -> dict:
    """Gera uma pergunta usando IA Gemini baseada no nível"""
    
    # Verifica se há perguntas no cache primeiro
    if questions_cache:
        cached = questions_cache.pop(0)
        print(f"📦 Usando pergunta do cache (restam {len(questions_cache)} no cache)")
        return cached
    
    # Se IA não estiver habilitada, retorna None para usar fallback externo
    ...
    # Seleciona temas aleatórios
    selected_themes = random.sample(temas, 3)
    
    # Prompt simplificado para gerar 3 perguntas
...
[
{{"question":"Pergunta sobre {selected_themes[0]}?","options":["opcao A","opcao B","opcao C","opcao D"],"correct":0}},
{{"question":"Pergunta sobre {selected_themes[1]}?","options":["opcao A","opcao B","opcao C","opcao D"],"correct":1}},
{{"question":"Pergunta sobre {selected_themes[2]}?","options":["opcao A","opcao B","opcao C","opcao D"],"correct":2}}
]
...
IMPORTANTE: Use aspas duplas, sem quebras de linha nas perguntas."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Valida e filtra perguntas válidas
            valid_questions = []
...
@app.get("/test_ai")
async def test_ai():
    """Endpoint para testar se a IA está funcionando"""
    if not AI_ENABLED or model is None:
...
async def get_question(
    ...
    """Retorna uma pergunta (do banco ou gerada por IA)"""
    try:
        ...
        # Tenta usar IA apenas se cache vazio e habilitado
        question = None
        ...
        # Se falhou ou IA desabilitada, usa banco de dados
        if not question:
            level_questions = QUESTIONS_DB[level]
            idx = question_num % len(level_questions)
...
async def get_ai_explanation(
    ...
    """Usa IA Gemini para explicar a resposta"""
